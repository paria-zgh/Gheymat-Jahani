...
function PriceUpdater() {
  ...
  const applyPrices = async () => {
    try {
      ...
      // پیدا کردن ستون‌ها و ردیف اختلاف
      let diffRow = -1;
      for (let r = 1; r <= ws.rowCount; r++) {
        for (let c = 1; c <= ws.columnCount; c++) {
          const v = normalize(getCellValue(ws.getCell(r, c)));
      ...
      if (productCol === -1 || date1Col === -1 || date2Col === -1)
        throw new Error("ستون‌های اصلی پیدا نشد!");
      if (diffCol === -1) throw new Error("ستون اختلاف پیدا نشد!");
      if (diffRow === -1) throw new Error("ردیف اختلاف پیدا نشد!");
      ...
      const updateSheet = (userData, userWb, targetCol) => {
        ...
        userData.forEach((r) => {
          ...
          if (normalizedName.includes("s 57")) {
            ...
            if (!map.has(normalizedName)) map.set(normalizedName, r.قیمت);
          }
        });
  
        const reversedDecimal = extractReversedDecimal(userWb.worksheets);
  
        for (let r = diffRow + 1; r <= ws.rowCount; r++) {
      ...
      // 🔹 مرتب‌سازی ردیف‌ها بعد از اولین ردیف دیتا
      const firstDataRow = diffRow + 1; // اولین ردیف دیتا بعد از ردیف اختلاف
      const rowsData = [];
      for (let r = firstDataRow + 1; r <= ws.rowCount; r++) { // شروع از ردیف دوم دیتا
        const val1 = Number(ws.getCell(r, date1Col).value || 0);
        const val2 = Number(ws.getCell(r, date2Col).value || 0);
        const diffCalc = val2 - val1;
  
        const rowValues = [];
        for (let c = 1; c <= ws.columnCount; c++)
          rowValues.push(ws.getCell(r, c).value);
  
        rowsData.push({ values: rowValues, diffCalc });
      }
  
      rowsData.sort((a, b) => b.diffCalc - a.diffCalc);
  
      rowsData.forEach((row, i) => {
        const targetRow = firstDataRow + 1 + i; // شروع از ردیف دوم دیتا
        for (let c = 1; c <= ws.columnCount; c++)
          ws.getCell(targetRow, c).value = row.values[c - 1];
      });
  
      // دوباره فرمول ستون اختلاف
      ...
      for (let r = diffRow + 1; r <= ws.rowCount; r++) {
        ws.getCell(r, diffCol).value = {
          formula: `${date2ColLetter}${r}-${date1ColLetter}${r}`,
        };
      }
  
      // Conditional Formatting ستون اختلاف
      ws.addConditionalFormatting({
        ref: `${diffColLetter}${diffRow + 1}:${diffColLetter}${ws.rowCount}`,
        rules: [
          {
            type: "cellIs",
            operator: "lessThan",
      ...
      // Conditional Formatting ستون درصد تغییر
      if (percentCol !== -1) {
        const percentColLetter = colNumToLetter(percentCol);
        ws.addConditionalFormatting({
          ref: `${percentColLetter}${diffRow + 1}:${percentColLetter}${ws.rowCount}`,
          rules: [
            {
              type: "cellIs",
              operator: "lessThan",
      ...
      const applyDateToBase = (label, value) => {
        ...
        for (let r = 1; r <= ws.rowCount; r++) {
          for (let c = 1; c <= ws.columnCount; c++) {
            ...
            if (normalize(getCellValue(cell)) === label) cell.value = value;
          }
        }
      };
